...
## Улучшение authoring workflow от 2026-07-29

- [x] Пользовательские названия `draft` заменены на `Не опубликован` и
  `Рабочая редакция`.
- [x] Неоднозначные поля `Порядок` переименованы в позиции внутри конкретных
  списков.
...
- [x] Добавлены официальные HTTPS URL как отдельный тип источника с
  SSRF/DNS-rebinding, redirect, MIME, size и timeout-защитой.
- [x] Добавлены предложения фактов с обязательным решением администратора;
  неподтверждённые кандидаты не участвуют в оценке и блокируют публикацию.
- [x] Для каждого вопроса показаны реально применимые подтверждённые факты и
  критерии.
- [x] Document, URL и fact-suggestion workers используют persisted jobs,
  ownership fencing, heartbeat, stale recovery и ограниченный shutdown.
- [x] Все мутации контента и публикация используют единый порядок блокировок
  `project → version`, чтобы публикация не пересекалась с невалидированным
  изменением.
- [ ] Реальные PostgreSQL/MinIO multi-replica race и restart-проверки пройдены
  на staging.
- [ ] Реальный TLS/DNS/redirect smoke официального URL выполнен на staging.
- [ ] Реальный provider smoke предложений фактов выполнен по утверждённому
  тестовому материалу и проектному ключу.
- [ ] Production deploy выполнен только после отдельного GO по
