...
- Отправлять испытуемому понятный итог проекта в Telegram после успешной или неуспешной сдачи.

Изменения:

- Для успешной сдачи итоговое сообщение заменено на `Поздравляем, проект (Название проекта) успешно сдан`.
- Для неуспешной сдачи итоговое сообщение заменено на `Проект (Название проекта) не сдан, осталось попыток (количество)`.
- Название берётся из связанного training project, остаток попыток вычисляется по лимиту зафиксированной версии проекта и числу использованных попыток.
- Предварительное состояние `REQUIRES_REVIEW` сохранило отдельное сообщение без преждевременного результата.
- После решения администратора создаётся отдельное идемпотентное outbox-событие с окончательным pass/fail; повтор того же review-запроса не дублирует сообщение.
- Интеграционные проверки Telegram-outbox обновлены для успешного результата, неуспешного результата, тайм-аута и восстановления worker после перезапуска.

Проверки:

- `pnpm build:api` - passed.
- `pnpm --filter @platforma/api test:unit` - 537/537 passed.
...
- Нет: автоматический результат и окончательный результат после ручной проверки используют один формирователь текста и отдельные идемпотентные outbox-события.
...
- Commit `e87aa2d` развернут в production; API-контейнер имеет статус
  `healthy`, `/health` возвращает `status=ok`, `database=ok`,
  `training=ready`.
- Production detail endpoint TATE возвращает `200`: проект остался `DRAFT`,
  присутствуют все `13` фактов, у каждого есть `questionLinks`, суммарно
  возвращаются все `27` связей.
- Визуальное повторное открытие вкладки «Факты» в пользовательском Safari
...
## 2026-07-28 — PDF-гайд по training admin для TATE

Что сделано:

- Создан короткий десятистраничный PDF-гайд по всем семи вкладкам редактора
  учебного проекта: назначение полей, правила заполнения, публикация и
  безопасный порядок запуска пилота.
- Визуальные схемы сверены с текущим production-кодом и read-only снимком
  данных TATE; production-данные не изменялись.
- В документе отдельно отмечено, что факты требуют ручной проверки, а цены,
  акции, наличие и обещания нельзя включать в scoring.

Файлы:

- `output/pdf/training-admin-tate-guide.pdf`.

Проверки:

- `pdfinfo`/`pypdf`: `10` страниц, A4 landscape, файл не зашифрован.
- `pdftoppm`: успешно отрендерены все `10` страниц в PNG; финальный
  contact sheet и плотные страницы проверены визуально, обрезаний и
  наложений не осталось.
- `pdftotext`: присутствуют заголовки всех семи вкладок и финальный чек-лист;
  Noto Sans/Noto Serif встроены в PDF.

Ручная проверка:

- Настоящие скриншоты production-интерфейса не добавлены: браузерное
  подключение было недоступно, поэтому использованы точные визуальные схемы.

Спорные места:

- Нет.

## 2026-07-28 — Три UX-концепции training content editor

Что сделано:

- Проведён read-only аудит компоновки вкладок «Основное», вопросов, «Факты»
  и «Критерии» в `TrainingAdminPage.tsx`, `trainingAdmin.css`, общем
  `field.tsx` и глобальных стилях формы.
- Подготовлены три визуальных варианта без изменения production-кода:
  компактный линейный редактор, master-detail и progressive disclosure.
- Во всех вариантах checkbox и подпись объединены в одну кликабельную строку,
  а двухколоночная сетка критериев заменена компоновкой без пустой области.

Файлы:

- `output/training-editor-design/options.html`.
- `output/training-editor-design/variant-a.png`.
- `output/training-editor-design/variant-b.png`.
- `output/training-editor-design/variant-c.png`.

Проверки:

- Все три варианта отрендерены headless Chrome в PNG `1920 x 1600`.
- Все три PNG проверены визуально.
- `git diff --check -- output/training-editor-design/options.html` — passed.

Ручная проверка:

- Выбрать один из трёх вариантов; реализация в приложении и browser QA
  намеренно не начинались до выбора.

Спорные места:

- Для master-detail потребуется защита несохранённого локального состояния
  при переключении элементов.
- Для progressive disclosure нужно заранее определить правило одновременного
  раскрытия и автораскрытия ошибок.

## 2026-07-28 — Training editor UX, вариант B master-detail

Что сделано:

- В режиме редактирования `Основное`, главный и дополнительные вопросы,
  `Факты` и `Критерии` переведены на единый master-detail: компактный реестр
  слева и один выбранный редактор справа.
- Все карточки внутри активной вкладки остаются смонтированными и
  переключаются через `hidden`, поэтому несохранённый локальный ввод не
  теряется при переходе между вопросами, фактами и критериями.
- `Основное` разделено на карточку проекта, доступность и настройки попытки;
  ошибка в скрытой группе автоматически открывает нужную панель.
- Добавлен локальный `TrainingCheckboxRow` со scoped reset native checkbox.
  Галочки активности, повторной попытки, подтверждения факта и связей
  вопросов теперь стоят непосредственно рядом с подписью и имеют общую
  кликабельную область.
- Связанные вопросы факта оформлены семантическим `fieldset`/`legend`,
  показывают номер, полный текст и статус выключенного вопроса.
- Две независимые колонки критериев удалены. Главные и дополнительные
  критерии объединены в сгруппированный реестр с количеством и суммой баллов;
  пустая правая область больше не возникает.
- После создания вопроса, факта или критерия сохраняется выбор реально
  созданной записи; URL, HTTP method и payload существующих API не менялись.
- На ширине до `900px` левый реестр заменяется нативным selector, detail
  остаётся одноколоночным без горизонтального scroll; добавление вопросов,
  фактов и критериев остаётся доступным рядом с selector.
...
- `apps/web/tests/training-admin-page.test.mjs`.
- `apps/web/tests-browser/training-admin-editor.spec.ts`.
- `apps/web/playwright.training.config.ts`.

Проверки:

- `pnpm --filter @platforma/web build` — passed.
- `pnpm --filter @platforma/web test` — `295/295`, passed.
- `pnpm --filter @platforma/web test:training:browser` — `14/14`, passed.
- Browser regression проверяет сохранность несохранённого draft при
  переключении, видимость ровно одной detail-панели, геометрию checkbox
  `16–24px`, зазор до текста не больше `16px`, отсутствие mobile overflow
  и mobile-действия добавления, а также доступность master-навигации в
  read-only версии при заблокированных полях.
- `pnpm test` — tools `23/23` и `64/64`, web `295/295`, API unit
  `444/444`; финальный общий прогон один раз получил транзиентный PostgreSQL
  deadlock в `two finalize workers produce one final result` (`89/90`).
- Немедленный изолированный повтор
  `pnpm --filter @platforma/api test:training:db:run` — passed, `90/90`;
  временная БД удалена runner-ом. Предыдущий полный прогон также был зелёным.
- `git diff --check` — passed.
- Светлая desktop, тёмная desktop и mobile `390px` версии отрендерены
  Playwright и проверены визуально.

Ручная проверка:

- На production ничего не разворачивалось и данные TATE не менялись.
- После отдельной команды на deploy проверить реальный TATE с полными
  списками `10` вопросов и `13` фактов, create/cancel/delete, read-only
  опубликованную версию, Safari и обе темы.
...
## 2026-07-31 — Переход к результатам из списка проектов обучения

Причина и исправление:

- Страница результатов и маршрут `/admin/training/results` уже существовали,
  но из списка проектов обучения к ним не было видимого перехода.
- В заголовок списка добавлена вторичная кнопка `Результаты`; она ведёт на
  существующий маршрут и показывается только при permission
  `training:results:read`.
- Основная кнопка `Создать проект`, логика оценки, API и данные не изменялись.

Проверки:

- Регрессионный source-contract test сначала воспроизвёл отсутствие перехода,
