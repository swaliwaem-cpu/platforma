...
- Registration: `LoginPage` in `apps/web/src/App.tsx` supports email code/token flow via `requestEmailRegistration()` and `verifyEmailRegistration()` from `apps/web/src/auth/AuthProvider.tsx`; `auth_token` query param is read in `LoginPage`.
