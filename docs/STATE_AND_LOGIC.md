...
- Cover state: пользователь может выбрать фото из добавленных ЖК или загрузить JPEG/PNG/WebP до 10 МБ; frontend показывает ошибку размера до запроса, backend повторно валидирует файл. Custom upload очищает `coverImageId`, а выбор фото ЖК освобождает `coverFileId` после versioned autosave.
