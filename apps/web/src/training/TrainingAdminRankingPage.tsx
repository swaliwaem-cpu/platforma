...
export function TrainingAdminRankingPage({ navigate }: { navigate: (pathname: string) => void }) {
  ...
  return <div className="training-page training-admin-page">
    <header className="training-page-header"><div><p className="eyebrow">Админка · Training V2</p><h2>Рейтинг сотрудников</h2><p className="muted-text">Исторические лучшие результаты и текущий охват доступных проектов.</p></div><div className="training-project-actions"><AdminButton type="button" tone="text" onClick={() => navigate('/admin/training')}>К проектам</AdminButton><AdminButton type="button" tone="primary" disabled={isExporting} onClick={() => void exportCsv()}>{isExporting ? 'Подготовка…' : 'Скачать CSV'}</AdminButton></div></header>
    <AdminPanel className="training-results-filter-panel"><form onSubmit={applyFilters}><FieldGroup className="training-ranking-filters"><Field><FieldLabel htmlFor="training-ranking-search">Сотрудник</FieldLabel><Input id="training-ranking-search" placeholder="Имя или email" value={draft.search} onChange={(event) => update('search', event.target.value)} /></Field><Field><FieldLabel htmlFor="training-ranking-project">Проект</FieldLabel><Input id="training-ranking-project" placeholder="Название или UUID" value={draft.project} onChange={(event) => update('project', event.target.value)} /></Field><SelectField id="training-ranking-access" label="Модель доступа" value={draft.accessMode} onChange={(value) => update('accessMode', value as TrainingAdminRankingQuery['accessMode'])} options={[["", "Все"], ["ASSIGNED_USERS", "По назначениям"], ["ALL_PARTICIPANTS", "Все участники"]]} /><SelectField id="training-ranking-assigned" label="Сейчас назначен" value={draft.currentlyAssigned} onChange={(value) => update('currentlyAssigned', value as TrainingAdminRankingQuery['currentlyAssigned'])} options={[["", "Все"], ["true", "Да"], ["false", "Нет"]]} /><SelectField id="training-ranking-eligible" label="Сейчас доступно" value={draft.currentlyEligible} onChange={(value) => update('currentlyEligible', value as TrainingAdminRankingQuery['currentlyEligible'])} options={[["", "Все"], ["true", "Да"], ["false", "Нет"]]} /></FieldGroup><div className="training-results-filter-actions"><AdminButton type="submit" tone="primary">Применить</AdminButton><AdminButton type="button" tone="text" onClick={resetFilters}>Сбросить</AdminButton></div></form></AdminPanel>
