...
- [ ] Перед началом реализации ещё раз проверить `git status --short`, если между сохранением плана и выполнением будут новые изменения.
- [ ] Не добавлять новые библиотеки, зависимости, сборщики или фреймворки.

## Решения

- [ ] Использовать фиксированный маппинг WordPress:
...
- [ ] Не использовать `korotkoe_opisanie` для публичного описания и новых полей.
- [ ] Не делать SQL backfill из старого `featuresJson.textSections`, чтобы не ошибиться при пустых исходных секциях; существующие объекты обновлять через повторный WordPress import.
- [ ] В публичной карточке показывать все три поля после описания; пустое значение отображать как `Не заполнено`.

## Implementation Checklist

### Этап 1. Контракт БД и API

- [ ] Добавить в `apps/api/prisma/schema.prisma` поля `RealEstateObject`:
...
- [ ] Создать миграцию `apps/api/prisma/migrations/20260511130000_add_object_content_sections/migration.sql` с тремя nullable `TEXT` колонками без backfill.
- [ ] Запустить `pnpm db:generate`.
- [ ] В `packages/shared/src/index.ts` добавить эти три поля в `RealEstateObjectBase` как `string | null`.

### Этап 2. `ObjectsService`

- [ ] В `CreateObjectBody` и `UpdateObjectBody` добавить:
  - `architectureDescription?: unknown`;
  - `infrastructureDescription?: unknown`;
  - `fillingDescription?: unknown`.
- [ ] В `create()` парсить поля через `parseNullableText(..., 10000)` и сохранять в `realEstateObject.create`.
- [ ] В `update()` обрабатывать очистку пустой строки в `null`, сравнение с текущим значением, `changes` и `hasScalarChanges`.
- [ ] В `serializeObjectBase()` вернуть три новых поля в API response.
- [ ] В `toAuditSnapshot()` добавить три новых поля.
- [ ] Не добавлять новые поля в publish requirements.

### Этап 3. WordPress Import

- [ ] В `tools/wp-import/src/types.ts` добавить поля в `MappedObject`:
  - `architectureDescription: string | null`;
  - `infrastructureDescription: string | null`;
  - `fillingDescription: string | null`.
- [ ] В `tools/wp-import/src/mapper.ts` изменить `buildDescription()`:
...
- [ ] В `tools/wp-import/src/importer.ts` добавить эти поля в `data` для create/update импортируемого объекта.
- [ ] Оставить `featuresJson` как технический JSON-след, но frontend больше не должен брать из него описание.

### Этап 4. Админка объекта

- [ ] В `apps/web/src/admin/ObjectsAdminPage.tsx` добавить поля в `ObjectFormState`, `emptyForm`, `createFormFromObject()` и `createPayloadFromForm()`.
- [ ] В секции `Основные данные` вставить после textarea `Описание` три textarea:
...
- [ ] Поле `Планировки и цены` оставить после новых textarea.
- [ ] В `validateObjectForm()` добавить лимит 10000 символов для каждого из трех полей.
- [ ] Не менять permissions, publish workflow, загрузку медиа/PDF и JSON-панель.

### Этап 5. Публичная карточка

- [ ] В `apps/web/src/objects/objectDetailViewModel.ts` добавить helper `getObjectContentSections(object)` с тремя строками: `Архитектура`, `Инфраструктура`, `Наполнение`.
- [ ] В `ObjectDetailPage.tsx` изменить `getDescriptionParagraphs()` так, чтобы он использовал только `object.description`.
- [ ] В JSX добавить секцию сразу после `Описание и особенности` и до `Файлы и документы`.
- [ ] Для каждого поля показывать label и абзацы текста; если значение пустое, показывать `Не заполнено`.
- [ ] В `apps/web/src/styles.css` добавить спокойную responsive-сетку для новых текстовых полей без декоративных hero/card-in-card эффектов.

### Этап 6. Тесты

- [ ] Расширить `apps/api/tests/services.test.cjs`:
...
- [ ] Обновить `tools/wp-import/tests/mapper.test.cjs`:
  - `description` равен `opisanie_2`;
  - три новых поля равны `opisanie_3`, `opisanie_4_1`, `opisanie_4_2`;
  - `korotkoe_opisanie` не используется.
- [ ] Обновить `apps/web/tests/short-description-removal.test.mjs`: публичное описание не должно fallback-иться к `object.shortDescription` или `featuresJson.textSections`.
- [ ] Добавить/расширить web static test, который проверяет наличие новых admin ids и публичного блока новых полей.

### Этап 7. Verification

- [ ] Запустить `pnpm db:generate`.
- [ ] Запустить `pnpm --filter @platforma/api test`.
- [ ] Запустить `pnpm --filter @platforma/wp-import test`.
- [ ] Запустить `pnpm build:web`.
- [ ] Запустить `pnpm wp-import:preview` с локальными WP env и проверить, что новые поля заполняются в preview без неожиданных warnings/errors.
- [ ] После подтверждения отдельно выполнить `pnpm wp-import:run`, чтобы существующие импортированные объекты получили новые поля и очищенное `description`.
